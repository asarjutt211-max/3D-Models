...
(function () {
  ...
  for (let i = 0; i < STAR_COUNT; i++) {
    // Distribution: 50% far, 35% mid, 15% near
    const r = Math.random();
  ...
  function drawTrail(dt) {
    for (let i = trailDots.length - 1; i >= 0; i--) {
      ...
      g.addColorStop(0, `rgba(${isTouch ? '232,255,0' : '232,255,0'},${d.life * 0.7})`);
  ...
  // Prevent gesture zoom (iOS pinch)
  ...
  function advanceLoader() {
    ...
    plFill.style.width = s.pct + '%';
    plText.textContent = s.text;
    stepIdx++;
    setTimeout(advanceLoader, 350 + Math.random() * 200);
  }
  ...
  let currentFps = 60;
  let perfMode = 'high'; // high, medium, low
  ...
  function loop(now) {
    ...
    if (now - fpsLastTime >= 500) {
      ...
      // Adaptive quality (check after initial stabilization)
      qualityCheckCount++;
      if (qualityCheckCount > 4 && isTouch) {
        if (currentFps < 40 && perfMode === 'high') {
          perfMode = 'medium';
          // Reduce star count by hiding every 3rd
    ...
    // Steering (touch gyro adds to it)
    ...
    // Warp chromatic aberration (RGB shift)
    if (warpBoost > 0.4) {
      ctx.save();
      ctx.globalCompositeOperation = 'screen';
      const shift = warpBoost * 4;
